...
void GoalPlannerModule::decideVelocity(PullOverPath & pull_over_path)
{
  ...
  // @LESLIE 2025-10-21 添加速度调试日志
  // ************************************************* //
  // [VEL_DEBUG] 记录速度决策
  RCLCPP_INFO(
    getLogger(),
    "[VEL_DEBUG][GoalPlanner][DECIDE] current_vel=%.3f, min_vel=%.3f, decided_vel=%.3f, path_points=%zu",
    current_vel,
    parameters_.pull_over_minimum_velocity,
    vel,
    first_path.points.size()
  );
  ...
  if (modified_count > 0) {
    RCLCPP_INFO(
      getLogger(),
      "[VEL_DEBUG][GoalPlanner][MODIFY] Modified %zu/%zu points velocity",
      modified_count,
      first_path.points.size()
    );
  }
