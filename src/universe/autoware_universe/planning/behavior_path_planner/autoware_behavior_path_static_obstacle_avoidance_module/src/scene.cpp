...
void StaticObstacleAvoidanceModule::insertAvoidanceVelocity(ShiftedPath & shifted_path) const
{
  ...
  if (path_shifter_.getShiftLines().empty()) {
    ...
    RCLCPP_DEBUG_THROTTLE(
      ...
      "[VEL_DEBUG][Avoidance][SKIP] No shift lines, skipping velocity insertion"
    );
    return;
  }
  ...
  if (data.target_objects.empty()) {
    ...
    RCLCPP_DEBUG_THROTTLE(
      getLogger(), *clock_, 2000,
      "[VEL_DEBUG][Avoidance][SKIP] No target objects, skipping velocity insertion"
    );
    return;
  }
  ...
  // @LESLIE
  // [VEL_DEBUG] 记录避障速度插入开始
  RCLCPP_INFO_THROTTLE(
    getLogger(), *clock_, 1000,
    "[VEL_DEBUG][Avoidance][INSERT] v_max=%.3f m/s, accel_end_dist=%.3f m, target_objs=%zu, ego_speed=%.3f m/s",
    v_max,
  ...
  if (distance_to_accel_end_point < 1e-3) {
    ...
    RCLCPP_WARN_THROTTLE(
      getLogger(), *clock_, 2000,
      "[VEL_DEBUG][Avoidance][SKIP] Distance too small (%.6f), skipping velocity insertion",
      distance_to_accel_end_point
    );
    return;
  }
  ...
  if (modified_count > 0) {
    RCLCPP_INFO_THROTTLE(
      getLogger(), *clock_, 1000,
      "[VEL_DEBUG][Avoidance][RESULT] Modified %zu points velocity",
      modified_count
    );
  }
