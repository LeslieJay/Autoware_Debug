...
void ObstacleCruisePlannerNode::onTrajectory(const Trajectory::ConstSharedPtr msg)
{
  ...
  *debug_data_ptr_ = DebugData();
  ...
  // @LESLIE
  // [VEL_DEBUG] 记录输出轨迹速度
  if (!output_traj.points.empty()) {
    const auto & first_point = output_traj.points.front();
    const auto & last_point = output_traj.points.back();
    const double input_first_vel = msg->points.empty() ? 0.0 : msg->points.front().longitudinal_velocity_mps;
    
    RCLCPP_INFO_THROTTLE(
      get_logger(), *get_clock(), 1000,
      "[VEL_DEBUG][ObstacleCruise][PUBLISH] Input_vel=%.3f -> Output_vel=%.3f (first), last_vel=%.3f, points=%zu",
      input_first_vel,
      first_point.longitudinal_velocity_mps,
      last_point.longitudinal_velocity_mps,
      output_traj.points.size()
    );
    
    // 记录巡航和减速限制
    if (cruise_vel_limit.has_value()) {
      RCLCPP_INFO_THROTTLE(
        get_logger(), *get_clock(), 1000,
        "[VEL_DEBUG][ObstacleCruise][LIMIT] Cruise limit: %.3f m/s, obstacles: %zu",
        cruise_vel_limit.value().max_velocity,
        cruise_obstacles.size()
      );
    }
    if (slow_down_vel_limit.has_value()) {
      RCLCPP_INFO_THROTTLE(
        get_logger(), *get_clock(), 1000,
        "[VEL_DEBUG][ObstacleCruise][LIMIT] Slow down limit: %.3f m/s, obstacles: %zu",
        slow_down_vel_limit.value().max_velocity,
