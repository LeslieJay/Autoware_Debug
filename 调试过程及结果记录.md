 ...
 * @LastEditTime: 2025-10-23 18:52:11
 * @Description: 
-->
...
![仿真环境下 /planning/scenario_planning/trajectory 话题输出](screen_shots/sim_planning_scen_traj.png) (screen_shots/sim_planning_scen_traj_2.png)
...
1. reference_path话题能够获取到speed_limit信息，说明地图没有问题，参考路径传递到 start planner这一步也没有问题，但是 start_planner组件的输出数据中，longitudinal_velocity_mps数据为0，可能是某些原因导致 start planner组件将longitudinal_velocity_mps数据设置为0。
...
### 如果起始点和目标点距离太近，无法获取有效车道，可能返回零速度路径
...
**文件：** `default_preset.yaml`   只保留 start planner 和 goal planner模块

**文件：** `scene_module_manager.param.yaml`   修改 start planner 和 goal planner模块的参数
